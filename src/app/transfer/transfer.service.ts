...
export class TransferService {
  ...
  /**
     ...
     * @returns
     */
  async transferToBankJuan(id: string, account: string, value: number, originAccount: number) {
    ...
    if (isNaN(value)) {
      throw new BadRequestException('El valor a transferir debe ser un número.');
    }

    // Validar que originAccount sea un número
    if (isNaN(originAccount)) {
      throw new BadRequestException('El número de cuenta de origen no es válido.');
    }
    ...
    if (saldo < value) {
      throw new BadRequestException(`No tiene fondos suficientes para realizar el retiro de la cuenta: ${originAccount}`);
    }
        ...
        this.httpService
          ...
          .pipe(catchError((error) => {
            if (error?.response?.status === 400) {
              ...
              throw new InternalServerErrorException('Error transferring', error);
            }
          }),
          ),
      );
      ...
      await firstValueFrom(
        this.httpService
          .post<any>(urlCompensation, payload, {
            ...
            catchError((error) => {
              throw new InternalServerErrorException('Error in compensation service', error);
            }),
          ),
      );
    } catch (error) {
